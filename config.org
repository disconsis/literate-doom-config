...
** TODO Save pdf on annot change
Auto-save PDFs on any annotation changes. This handles adding, deleting, and editing annotations. I find myelf constantly typing ~:wq~   without this.
The code below doesn't quite work - figure out why.
#+begin_src emacs-lisp
(after! pdf-tools
  (advice-add #'pdf-annot-edit-contents-commit
              :after (cmd! (save-buffer))))
#+end_src
