...
Changing the theme with =counsel-load-theme= has one downside - you can't see the currently active theme. We'll make a slight modification to change that.
NOTE: I wanted to use the =[remap counsel-load-theme]= syntax here to make the keybinding independent of whatever =counsel-load-theme= is actually bound to, but I couldn't get that to work.
#+begin_src emacs-lisp
...
Usable with `ivy-resume', `ivy-next-line-and-call' and `ivy-previous-line-and-call'."
  (interactive)
  (ivy-read (format "Load custom theme [current: %s]: " (s-join ", " (mapcar #'symbol-name custom-enabled-themes)))
            (mapcar 'symbol-name
...
(map! :map doom-leader-map
      "h t" #'my/counsel-load-theme-showing-current)
#+end_src
...
Similarly, headings by default are just bolded. Since the default variable-pitch font is sans-serif, I thought it might be nice to mix it up with a serif font for the headings.
#+begin_src emacs-lisp
